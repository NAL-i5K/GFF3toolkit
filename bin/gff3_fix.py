...
if __name__ == '__main__':
    ...
    logger_stderr.infor('Reading QC report file: (%s)...\n', args.qc_report)
            ...
            for line in qcr:
                ...
                for line_num in line_num_list:
                    if line_num not in line_num_dict:
                        line_num_dict[line_num] = {lines[1]: [line[2]]}
                    else:
                        if line[1] not in line_num_dict[line_num]:
                            line_num_dict[line_num][line[1]] = [line[2]]
                        else:
                            line_num_dict[line_num][line[1]].append(line[2])

    except:
        logger_stderr.error('Failed to read QC report file!')
    logger_stderr.infor('Reading GFF3 file: (%s)...\n', args.gff)
    try:
        gff3 = Gff3(gff_file=args.gff, logger=logger_null)
    except:
        logger_stderr.error('Failed to read GFF3 file!')
    gff3_fix.fix.main(gff3=gff3, output_gff=args.out_gff, report=args.report, error_dict=error_dict, line_num_dict=line_num_dict, logger=logger_null)
